...
$(function(){
	...
	$("#mainjoinInsert").click(function(){
		//입력값 체크
		if (!formCheck($('#m_id'), $('.error'), "아이디를"))	return;
		else if (idConfirm!=2){ alert("아이디 중복 체크 진행해 주세요."); return;}
		else if (!inputVerify(0,'#m_id', '.error'))	return;
		else if (!formCheck($('#m_pw'), $('.error1'), "비밀번호를"))	return;
		else if (!inputVerify(1,'#m_pw', '.error1'))	return;
		...
		else if (!formCheck($('#m_name'), $('.error3'), "이름을"))	return;
		else { 
			$(".mainForm").attr({
				"method":"post",
