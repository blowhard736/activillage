...
public class FileUploadUtil {	
	...
	/* +++++++++++++++++++++++기타 파일 업로드 메서드 */
	public static String fileExUpload(MultipartFile fileEx, HttpServletRequest request, String fileExName) throws IOException{
		log.info("fileExUpload 호출 성공 ");
		
		String	real_name=null;
		// MultipartFile 클래스의 getFile() 메서드로 클라이언트가 업로드한 파일
		String org_name = fileEx.getOriginalFilename();
		log.info("org_name :" +org_name);
		
		// 파일명 변경(중복되지 않게) 
		if(org_name != null && (!org_name.equals(""))){
			real_name = fileExName +"_"+ System.currentTimeMillis() +"_"+ org_name; // 저장할 파일 이름
			
			String docRoot = request.getSession().getServletContext().getRealPath("/uploadStorage/"+fileExName);
			makeDir(docRoot);
				
			File fileAdd = new File(docRoot+"/"+real_name);	//파일 생성후 
			log.info("업로드할 파일(fileAdd) : " + fileAdd);
			
			fileEx.transferTo(fileAdd); 
		}
		return real_name;
	}
